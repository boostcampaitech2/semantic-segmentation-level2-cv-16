...
def train(data_dir, model_dir, args):
    ...
    train_transform = A.Compose(
        [A.Rotate(limit=90), A.RandomResizedCrop(512, 512), ToTensorV2()]
    )
    ...
    model_path = "/opt/ml/segmentation/semantic-segmentation-level2-cv-16/dev/model_develop/HRNet/HRNetV2_W64_OCR/result/54-2_/54-2__HRNetV2_W64_OCR_115_0.6898.pth"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--name", default="54_4", help="model save at {SM_MODEL_DIR}/{name}"
    )
    ...
    wandb.run.name = (
        f"{args.name}_(54_2+pseudo)_HRNetV2_W64_OCR_cos_rot+randomresizecrop"
    )
