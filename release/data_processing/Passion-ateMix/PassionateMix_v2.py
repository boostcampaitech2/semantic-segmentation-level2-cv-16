...
def save_merged_image(save_dir, file_name_dir, merged_image, bg_image_infos, fg_image_infos, mask_class):
    if not os.path.isdir(save_dir):                                                           
        os.mkdir(save_dir)

    merged_image = cv2.cvtColor(merged_image, cv2.COLOR_BGR2RGB)
    cv2.imwrite(save_dir+str(mask_class)+'_'+str(fg_image_infos['id'])+'_'+'_'.join(bg_image_infos['file_name'].split('/')), merged_image)
    
    
def make_coco_annotation(file_name_dir,bg_image_infos, fg_image_infos, mask_class, segmentation_mask, json_data):
    # 수정해야할 image_id
    revised_id = bg_image_infos['id']

    # json으로 읽어들일 image 수정
    json_data['images'][revised_id]['file_name'] = file_name_dir + str(mask_class)+'_'+str(fg_image_infos['id'])+'_'+'_'.join(bg_image_infos['file_name'].split('/'))
